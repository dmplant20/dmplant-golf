...
const {data:u}=await a.from('users').select('id,full_name,email').eq('full_name','신종섭').maybeSingle()
if(!u){ console.error('❌ 신종섭 user 없음'); process.exit(1) }
const {data:pres}=await a.from('users').select('id').eq('full_name','최성복').single()

// fee_type=monthly, joined Jan
const {error:mErr}=await a.from('club_memberships')
  .update({ status:'approved', fee_type:'monthly', joined_at:'2026-01-01' })
  .eq('club_id', c.id).eq('user_id', u.id)
if(mErr){ console.error('membership:', mErr.message); process.exit(1) }
console.log('✓ 신종섭 멤버십: approved, monthly, 2026-01-01')
...
const {data:final}=await a.from('finance_transactions')
  ...
  .eq('club_id', c.id).eq('member_id', u.id).eq('type','fee').order('transaction_date')
console.log('\n신종섭 회비 내역:')
