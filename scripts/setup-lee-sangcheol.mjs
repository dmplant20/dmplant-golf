...
const TARGET = {
  name:  '이상철',
  en:    'Lee Sangcheol',
  email: 'sclee@wintrating.co.kr',
}
const PASSWORD = 'lee1234'

console.log(`━━━ ${TARGET.name} 설정 (email=${TARGET.email}) ━━━`)

// 1) public.users 에서 이름으로 검색
console.log('\n1) public.users 검색:')
...
if (byName && byName.length > 0) {
  const u = byName[0]
  userId = u.id
  console.log(`  ✓ 발견 — id=${userId.slice(0,8)}, 현재 email=${u.email}`)
} else {
  console.log('  · 이름으로 못 찾음')
}

// 2) 새 이메일로도 검색 (이미 다른 user_id 로 가입돼있는지)
console.log('\n2) 새 이메일로 검색:')
...
if (byEmail) {
  ...
  if (userId && userId !== byEmail.id) {
    console.log('  ⚠ 같은 사람이 두 user_id — 새 이메일 user 우선 사용')
    userId = byEmail.id
  } else if (!userId) {
    userId = byEmail.id
  }
} else {
  console.log('  · 새 이메일 등록 user 없음')
}
...
if (userId) {
  ...
  if (authBefore?.user) {
    ...
    if (uErr) { console.log(`  ❌ auth 업데이트 실패: ${uErr.message}`); process.exit(1) }
    console.log(`  ✓ auth 이메일 → ${TARGET.email}, 비번 → ${PASSWORD}`)
  } else {
    console.log(`  ❌ public.users 행은 있는데 auth.users 에 없음 — 신규 생성`)
...
// 4) public.users 갱신
console.log('\n4) public.users 갱신:')
const { error: pErr } = await admin.from('users').update({
  email: TARGET.email,
  full_name: TARGET.name,
  full_name_en: TARGET.en,
  password_set: true,   // 사용자가 명시한 비번이므로 첫로그인 팝업 스킵
}).eq('id', userId)
if (pErr) console.log(`  ⚠ users 갱신 경고: ${pErr.message}`)
else console.log(`  ✓ email/full_name/full_name_en/password_set=true 갱신`)
